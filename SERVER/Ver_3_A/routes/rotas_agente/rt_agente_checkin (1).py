    ...
    try:
        ...
        if system_info:
            ...
            if 'installed_software' in system_info:
                print(f"Iniciando sincronização completa de software para asset {asset.id}")
                
                # 1. Obter softwares atualmente no banco para este asset
                ...
                for sw in current_software_in_db:
                    # Criar uma chave única para comparação (nome + versão + vendor)
                    key = f"{sw.name}|{sw.version or ''}|{sw.vendor or ''}"
                    current_software_set.add(key)
                
                print(f"Softwares atualmente no banco: {len(current_software_set)}")
                ...
                print(f"Softwares reportados pelo agente: {len(agent_software_set)}")
                print(f"Novos softwares para adicionar: {software_added_count}")
                
                # 3. Identificar softwares que foram desinstalados
                ...
                print(f"Softwares para remover (desinstalados): {software_removed_count}")
                
                # 4. Remover softwares desinstalados
                if software_removed_list:
                    for sw in software_removed_list:
                        print(f"Removendo software desinstalado: {sw.name} {sw.version}")
                        db.session.delete(sw)
                    
                    # Registrar no histórico
                    removed_names = [f"{sw.name} {sw.version}" for sw in software_removed_list]
                    history = AssetHistory(
                        asset_id=asset.id,
                        user_id=None,
                        action='software_uninstalled',
                        new_value=f'Softwares desinstalados removidos: {", ".join(removed_names[:5])}{"..." if len(removed_names) > 5 else ""}'
                    )
                    db.session.add(history)
                ...
                if software_to_insert:
                    try:
                        ...
                        actual_added = result.rowcount
                        
                        # Registrar no histórico
                        if actual_added > 0:
                            added_names = [sw['name'] for sw in software_to_insert[:5]]
                            history = AssetHistory(
                                asset_id=asset.id,
                                user_id=None,
                                action='software_installed',
                                new_value=f'Novos softwares instalados: {", ".join(added_names)}{"..." if len(software_to_insert) > 5 else ""}'
                            )
                            db.session.add(history)
                ...
                # 6. Registrar resumo da sincronização
                total_changes = software_added_count + software_removed_count
                if total_changes > 0:
                    summary_history = AssetHistory(
                        asset_id=asset.id,
                        user_id=None,
                        action='software_sync_complete',
                        new_value=f'Sincronização completa: {software_added_count} adicionados, {software_removed_count} removidos, {len(agent_software_set)} total'
                    )
                    db.session.add(summary_history)
                
                print(f"Sincronização completa finalizada para asset {asset.id}: {software_added_count} adicionados, {software_removed_count} removidos")
            ...
            if 'windows_updates' in system_info and system_info['windows_updates'].get('updates'):
                current_updates_on_asset = {
                    wu.hotfix_id for wu in asset.windows_updates_entries if hasattr(asset, 'windows_updates_entries')
                }
